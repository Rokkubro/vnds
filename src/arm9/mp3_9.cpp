...
void mp3_fill_buffer() {
	...
	//mp3_print();
	if(mp3 && mp3->flag) {
		n = fhRead((void *)(mp3->buffer + MP3_FILE_BUFFER_SIZE), 1, MP3_FILE_BUFFER_SIZE, mp3->file);
		filled += n;
		if(mp3->loop && n < MP3_FILE_BUFFER_SIZE) {
			fhSeek (mp3->file, 0, SEEK_SET);
			n = fhRead((void *)(mp3->buffer + MP3_FILE_BUFFER_SIZE + n), 1, MP3_FILE_BUFFER_SIZE-n, mp3->file);
			filled += n;
		}
		mp3->flag = 0;
	}
...
int ds_filelength (FileHandle *fh)
{
	int		pos;
	int		end;

	pos = fhTell (fh);
	fhSeek (fh, 0, SEEK_END);
	end = fhTell (fh);
	fhSeek (fh, pos, SEEK_SET);

	return end;
}

int mp3_play_file(FileHandle *file,int loop){
	...
	filled += fhRead((void *)(mp3->buffer), 1, MP3_FILE_BUFFER_SIZE*2, mp3->file);
...
int mp3_play(char *filename,int loop){
	...
	FileHandle *file = fhOpen(filename);
